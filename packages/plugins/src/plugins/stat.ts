    ...
    bot.addCommand({
        ...
        cb: async (msg, client: Client & any) => {
            ...
            if (msg.args.length === 0) {
                ...
                if (!id) {
                    client.send("Unable to find user. This can happen if they have not been in the room in awhile.", msg);
                    return;
                }
            }
            ...
            client.send(client.codify("" +
                ...
                `Last Accessed: ${(new Date(userData.last_access_date * 1000))}`), msg);
        }
    });
};
