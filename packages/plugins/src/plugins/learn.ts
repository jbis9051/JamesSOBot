  ...
  bot.addCommand({
    ...
    cb: async (msg, client) => {
      if (
        ...
        && !(learn_list.hasOwnProperty(msg.args[0]) && learn_list[msg.args[0]].creatorID === msg.info.fromId) /* unless its a learned command and you have permission to overwrite it */
      ) {
        client.hardReply("Command with that shortcut already exists", msg);
        return;
      }
    ...
    function addLearnCommand(learn_object: LearnItem) {
        bot.addCommand({
          ...
          cb: (msg, client) => {
            ...
            let promise;
            if (output.match(/^<msg>/)) {
              promise = client.hardReply(output.replace(/^<msg>/, ""), msg);
            } else {
