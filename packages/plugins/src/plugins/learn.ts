...
export const learn: PluginFunction = (bot: Bot) => {
  const learn_list: { [key: string]: LearnItem } =
    bot.dataSt"learn_list"'learn_list') || {};
  Object.values(learn_list).forEach(addLearnCommand);
  bot.addCommand("learn"ame: 'learn',"shortcut" ["output"t', 'output'],
    descrip"Teaches a bot a command. Will output the `output` when `|| shortcut` is called. You can also add args by wrapping the arg number (starting with 1) in curly brackets. If you would like to escape spaces (like for a link) wrap the index in regular brackets. You can also use `{a}` to include all the arguments and `[a]` to encode them all."e them all.',
    "learn"ts: ['learn'],
    exampl"|| learn shortcut output"tcut out"|| learn tbh to be honest"o be hon"|| learn hbd Happy Birthday {1}!"rthday {"|| learn vampire_redirect https://lmgtfy.com/?q=[1]"com/?q=[1]',
    ],
    ignore: false,
    ...
    cb: async (msg, client) => {
      if (
        ...
        client.hardReply"Command with that shortcut already exists"eady exists',
          msg
        );
        return;
      }
      if (msg.args.length < 2) {
        "Invalid Number of args"ber of args', msg);
        return;
      }

      const name = msg.args.shift() as string;
      const output = bot.htmlToMarkdown(ms" "rgs.join(' '));
      ...
      bot.dataSt"learn_list"'learn_list', learn_list);
      client." has been added" been added',msg);
    },
  });
