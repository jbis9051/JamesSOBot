    ...
    bot.addCommand({
        ...
        cb: (msg, client) => {
            if (msg.quotedArgsList.length < 1) {
                return client.send("Please add an arg for something to poll", msg);
            }
            if (msg.quotedArgsList[0] === "status") {
                return client.send("**Status**: " + polls[msg.info.contextId].resultsSummary(), msg)
            }
            if (msg.quotedArgsList[0] === "close") {
                ...
                if (poll.creator.id !== msg.info.fromId && !client.isRoomOwnerId(msg.info.fromId, msg)) {
                    return client.send("You did not create this poll so you can't close it.", msg);
                }
                poll.close();
            ...
            if (polls.hasOwnProperty(msg.info.contextId)) {
                return client.send("A poll is already open for this room. Use `|| vote [choice]` to vote and `|| vote` to see options: " + polls[msg.info.contextId].query, msg);
            }
            polls[msg.info.contextId] = new Poll(msg.quotedArgsList.shift()!, msg.quotedArgsList, {
                name: msg.info.fromName,
                id: msg.info.fromId
            }, () => closePoll(msg, client));
            client.send("**New Poll Created**: " + polls[msg.info.contextId].query, msg).then(_ =>
                client.send("Choices: " + polls[msg.info.contextId].choiceSummary(), msg)
            );
        }
    });
    ...
    bot.RegisterShutdownScript((msg, client) => {
        return Promise.all(
            ...
            Object.values(polls).map(async ([room, poll]) => {
                await client.send("**Bot Shutdown Imminent. Polls Auto-Closing**", room);
                await client.send("**Poll Closed**: " + poll.query, room);
                await client.send("**Results**: " + poll.resultsSummary(), room);
            }))
    });
};
...
function closePoll(msg: Message, client: Client) {
    ...
    return client.send("**Poll Closed**: " + poll.query, msg).then(_ =>
        client.send("**Results**: " + poll.resultsSummary(), msg)
    );
}
...
class Poll {
    ...
    constructor(query: string, choices: string[], creator: { id: string, name: string }, timeoutCallback: () => void, timeout = 14400000) {
        this.query = query;
