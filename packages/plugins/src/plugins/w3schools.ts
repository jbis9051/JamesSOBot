...
export const w3schools: PluginFunction = (bot) => {
    ...
    bot.RegisterHandler(async (msg, client) => {
      ...
      const noMDNReply = `w3schools is a terrible resource. We suggest using ${client.link("MDN", "https://developer.mozilla.org/")}.`;
      ...
      bot.google_search(title, "developer.mozilla.org", undefined, /^https:\/\/developer\.mozilla\.org\/.*$/).then((data) => {
        if (data) {
          client.hardReply(`w3schools is a terrible resource. We suggest using MDN. Here's an potentially equivalent page: ${bot.htmldecode(client.link(data.title, data.url))}`, msg);
        } else {
          client.hardReply(noMDNReply, msg);
        }
      });
